                    ...
                    <div class="layui-row" align="center" style="height: 40px">
                        ...
                        <form class="layui-form layui-col-md12 x-so" method="get">
                            <input type="text" id="queryContent" name="content" style="width:50%;"
                                   placeholder="请输入用户姓名，宠物名，评论，或者时间来查询..."
...
<script>
    layui.config({
                ...
                case 'getCheckData':
                    ...
                    if (ids.length == 0) {
                        ...
                        layer.alert('确认删除这' + data.length + '条数据吗？', function (index) {
                            layer.close(index)
                            $.post("/comment/delBatches", {ids}, function (res) {
        ...
        table.on('tool(test-table-toolbar)', function (obj) {
            ...
            if (obj.event === 'del') {
                ...
                layer.open({
                    ...
                    content: "/comment/toUpd?id=" + data.id
                });
        ...
        $('#queryRole').on('click', function (event, n) {
            ...
            if (n) {
                tableIns.reload({
                    url: '${pageContext.request.contextPath}/comment/list?content=' + queryContent
                })
                return;
            }
            tableIns.reload({
